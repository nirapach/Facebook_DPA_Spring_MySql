...
import com.gravity4.facebook.dpareports.CSVFileWriter.Aggregated.AggregatedCampaignCSVWriter;
import com.gravity4.facebook.dpareports.dao.AggregatedandAgeandGender.OverAllCampaignLevelStatsDAO;
import com.gravity4.facebook.dpareports.model.CSVModels.Aggregated.CSVAggregatedCampaignStats;
import com.gravity4.facebook.dpareports.model.APImodels.AggregatedandProductLevel.CampaignStatsLoader;
import com.gravity4.facebook.dpareports.responseparser.responsedata.Aggregated.AggregatedCampaignStatsJSONResponse;
...
public class AggregatedCampaignStats {
    ...
    Logger logger = LoggerFactory.getLogger(AggregatedCampaignStats.class);
    ...
    /*
        ...
        */
    public String getOverAllCampaignstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_group_name','spend','total_actions'," +
         ...
         */
            if (status) {
                ...
                AggregatedCampaignStatsJSONResponse response = gson.fromJson(jsonfeed, AggregatedCampaignStatsJSONResponse.class);

                List<AggregatedCampaignResultData> results = response.data;
                ...
                for (AggregatedCampaignResultData resultData : results) {
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    campaignStatsLoader.setGender("");

                    campaignStatsLoaderList.add(campaignStatsLoader);


                }
                overAllCampaignLevelStatsDAO.storeaggregatedcamapignlevelstats(campaignStatsLoaderList);

                //for storing into file for email
                List<CSVAggregatedCampaignStats> csvAggregatedCampaignStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvAggregatedCampaignStatsList=overAllCampaignLevelStatsDAO.fileaggregatedadsetlevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= AggregatedCampaignCSVWriter.writecsvfile(csvAggregatedCampaignStatsList, Client_ID, emailstatsdate,csv_address);
