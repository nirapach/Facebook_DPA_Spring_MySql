...
import com.gravity4.facebook.dpareports.CSVFileWriter.Placement.PlacementAdGroupCSVWriter;
import com.gravity4.facebook.dpareports.dao.Placement.PlacementAdGroupLevelStatsDAO;
...
import com.gravity4.facebook.dpareports.responseparser.responsedata.Placement.PlacementAdGroupStatsJSONResponse;
...
public class PlacementAdGroupStats {

    Logger logger = LoggerFactory.getLogger(PlacementAdGroupStats.class);

    @Autowired
    PlacementAdGroupLevelStatsDAO placementAdGroupLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getPlacementAdGroupstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['adgroup_name','spend','placement','impression_device','total_actions'," +
         ...
         */
            if (status) {
                ...
                PlacementAdGroupStatsJSONResponse response = gson.fromJson(jsonfeed, PlacementAdGroupStatsJSONResponse.class);

                List<PlacementAdGroupResultData> results = response.data;

                PlacementAdGroupStatsLoader adGroupStatsLoader;
                List<PlacementAdGroupStatsLoader> adGroupStatsLoaderList = new ArrayList<PlacementAdGroupStatsLoader>();

                for (PlacementAdGroupResultData resultData : results) {

                    adGroupStatsLoader = new PlacementAdGroupStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                ...
                placementAdGroupLevelStatsDAO.storeadgrouplevelstats(adGroupStatsLoaderList);

                //for storing into file for email
                List<CSVPlacementAdGroupStats> csvPlacementAdGroupStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvPlacementAdGroupStatsList=placementAdGroupLevelStatsDAO.fileadgrouplevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= PlacementAdGroupCSVWriter.writecsvfile(csvPlacementAdGroupStatsList, Client_ID, emailstatsdate,csv_address);
