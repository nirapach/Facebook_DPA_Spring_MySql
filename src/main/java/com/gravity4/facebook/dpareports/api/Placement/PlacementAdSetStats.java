...
import com.gravity4.facebook.dpareports.CSVFileWriter.Placement.PlacementAdSetCSVWriter;
import com.gravity4.facebook.dpareports.dao.Placement.PlacementAdSetLevelStatsDAO;
...
import com.gravity4.facebook.dpareports.responseparser.responsedata.Placement.PlacementAdSetStatsJSONResponse;
...
public class PlacementAdSetStats {

    Logger logger = LoggerFactory.getLogger(PlacementAdSetStats.class);


    @Autowired
    PlacementAdSetLevelStatsDAO placementAdSetLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getPlacementAdSetstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_name','spend','placement','impression_device','total_actions'," +
         ...
         */
            if (status) {
                ...
                PlacementAdSetStatsJSONResponse response = gson.fromJson(jsonfeed, PlacementAdSetStatsJSONResponse.class);

                List<PlacementAdSetResultData> results = response.data;

                PlacementAdSetStatsLoader adSetStatsLoader;
                List<PlacementAdSetStatsLoader> adSetStatsLoaderList = new ArrayList<PlacementAdSetStatsLoader>();

                for (PlacementAdSetResultData resultData : results) {

                    adSetStatsLoader = new PlacementAdSetStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                ...
                placementAdSetLevelStatsDAO.storeadsetlevelstats(adSetStatsLoaderList);


                //for storing into file for email
                List<CSVPlacementAdSetStats> csvPlacementAdSetStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvPlacementAdSetStatsList=placementAdSetLevelStatsDAO.fileadsetlevelstats(Client_ID,emailstatsdate);
                //call the CSV file writer
                String stored= PlacementAdSetCSVWriter.writecsvfile(csvPlacementAdSetStatsList, Client_ID, emailstatsdate,csv_address);
