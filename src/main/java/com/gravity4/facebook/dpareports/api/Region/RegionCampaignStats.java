...
import com.gravity4.facebook.dpareports.CSVFileWriter.Region.RegionCampaignCSVWriter;
import com.gravity4.facebook.dpareports.dao.Region.RegionCampaignLevelStatsDAO;
...
import com.gravity4.facebook.dpareports.responseparser.responsedata.Region.RegionCampaignStatsJSONResponse;
...
public class RegionCampaignStats {

    Logger logger = LoggerFactory.getLogger(RegionCampaignStats.class);


    @Autowired
    RegionCampaignLevelStatsDAO regionCampaignLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getRegionCampaignstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_group_name','spend','region','total_actions'," +
         ...
         */
            if (status) {
                ...
                RegionCampaignStatsJSONResponse response = gson.fromJson(jsonfeed, RegionCampaignStatsJSONResponse.class);

                List<RegionCampaignResultData> results = response.data;
                ...
                for (RegionCampaignResultData resultData : results) {
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    campaignStatsLoader.setRegion(resultData.region);
                    ...
                    campaignStatsLoader.setCountry("");

                    campaignStatsLoaderList.add(campaignStatsLoader);


                }
                regionCampaignLevelStatsDAO.storecamapignlevelstats(campaignStatsLoaderList);

                //for storing into file for email
                List<CSVRegionCampaignStats> csvRegionCampaignStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvRegionCampaignStatsList=regionCampaignLevelStatsDAO.fileadsetlevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= RegionCampaignCSVWriter.writecsvfile(csvRegionCampaignStatsList, Client_ID, emailstatsdate,csv_address);
