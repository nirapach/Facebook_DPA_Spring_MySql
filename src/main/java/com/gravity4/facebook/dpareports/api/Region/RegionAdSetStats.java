...
import com.gravity4.facebook.dpareports.CSVFileWriter.Region.RegionAdSetCSVWriter;
import com.gravity4.facebook.dpareports.dao.Region.RegionAdSetLevelStatsDAO;
import com.gravity4.facebook.dpareports.model.CSVModels.Region.CSVRegionAllAdSetStats;
import com.gravity4.facebook.dpareports.model.APImodels.CountryandRegion.CountryandRegionAdSetStatsLoader;
import com.gravity4.facebook.dpareports.responseparser.responsedata.Region.RegionAdSetStatsJSONResponse;
...
public class RegionAdSetStats {

    Logger logger = LoggerFactory.getLogger(RegionAdSetStats.class);


    @Autowired
    RegionAdSetLevelStatsDAO regionAdSetLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getRegionAdSetstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_name','spend','region','total_actions'," +
         ...
         */
            if (status) {
                ...
                RegionAdSetStatsJSONResponse response = gson.fromJson(jsonfeed, RegionAdSetStatsJSONResponse.class);

                List<RegionAdSetResultData> results = response.data;

                CountryandRegionAdSetStatsLoader adSetStatsLoader;
                List<CountryandRegionAdSetStatsLoader> adSetStatsLoaderList = new ArrayList<CountryandRegionAdSetStatsLoader>();

                for (RegionAdSetResultData resultData : results) {

                    adSetStatsLoader = new CountryandRegionAdSetStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    adSetStatsLoader.setRegion(resultData.region);
                    ...
                    adSetStatsLoader.setCountry("");

                    adSetStatsLoaderList.add(adSetStatsLoader);


                }
                regionAdSetLevelStatsDAO.storeadsetlevelstats(adSetStatsLoaderList);


                //for storing into file for email
                List<CSVRegionAllAdSetStats> csvRegionAllAdSetStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvRegionAllAdSetStatsList=regionAdSetLevelStatsDAO.fileadsetlevelstats(Client_ID,emailstatsdate);
                //call the CSV file writer
                String stored= RegionAdSetCSVWriter.writecsvfile(csvRegionAllAdSetStatsList, Client_ID, emailstatsdate,csv_address);
