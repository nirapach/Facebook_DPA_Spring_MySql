...
import com.gravity4.facebook.dpareports.CSVFileWriter.Region.RegionAdGroupCSVWriter;
import com.gravity4.facebook.dpareports.dao.Region.RegionAdGroupLevelStatsDAO;
...
import com.gravity4.facebook.dpareports.responseparser.responsedata.Region.RegionAdGroupStatsJSONResponse;
...
public class RegionAdGroupStats {

    Logger logger = LoggerFactory.getLogger(RegionAdGroupStats.class);

    @Autowired
    RegionAdGroupLevelStatsDAO regionAdGroupLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getRegionAdGroupstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['adgroup_name','spend','region','total_actions'," +
         ...
         */
            if (status) {
                ...
                RegionAdGroupStatsJSONResponse response = gson.fromJson(jsonfeed, RegionAdGroupStatsJSONResponse.class);

                List<RegionAdGroupResultData> results = response.data;
                ...
                for (RegionAdGroupResultData resultData : results) {
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    adGroupStatsLoader.setRegion(resultData.region);
                    ...
                    adGroupStatsLoader.setCountry("");

                    adGroupStatsLoaderList.add(adGroupStatsLoader);


                }
                regionAdGroupLevelStatsDAO.storeadgrouplevelstats(adGroupStatsLoaderList);

                //for storing into file for email
                List<CSVRegionAdGroupStats> csvRegionAdGroupStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvRegionAdGroupStatsList=regionAdGroupLevelStatsDAO.fileadgrouplevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= RegionAdGroupCSVWriter.writecsvfile(csvRegionAdGroupStatsList, Client_ID, emailstatsdate,csv_address);
