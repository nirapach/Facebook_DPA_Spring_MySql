...
import com.gravity4.facebook.dpareports.CSVFileWriter.Placement.PlacementCampaignCSVWriter;
import com.gravity4.facebook.dpareports.dao.Placement.PlacementCampaignLevelStatsDAO;
...
import com.gravity4.facebook.dpareports.responseparser.responsedata.Placement.PlacementCampaignStatsJSONResponse;
...
public class PlacementCampaignStats {

    Logger logger = LoggerFactory.getLogger(PlacementCampaignStats.class);


    @Autowired
    PlacementCampaignLevelStatsDAO placementCampaignLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getPlacementCampaignstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_group_name','spend','placement','impression_device','total_actions'," +
         ...
         */
            if (status) {
                ...
                PlacementCampaignStatsJSONResponse response = gson.fromJson(jsonfeed, PlacementCampaignStatsJSONResponse.class);

                List<PlacementCampaignResultData> results = response.data;

                PlacementCampaignStatsLoader campaignStatsLoader;
                List<PlacementCampaignStatsLoader> campaignStatsLoaderList = new ArrayList<PlacementCampaignStatsLoader>();

                for (PlacementCampaignResultData resultData : results) {

                    campaignStatsLoader = new PlacementCampaignStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                ...
                placementCampaignLevelStatsDAO.storecamapignlevelstats(campaignStatsLoaderList);

                //for storing into file for email
                List<CSVPlacementCampaignStats> csvPlacementCampaignStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvPlacementCampaignStatsList=placementCampaignLevelStatsDAO.fileadsetlevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= PlacementCampaignCSVWriter.writecsvfile(csvPlacementCampaignStatsList, Client_ID, emailstatsdate,csv_address);
