package com.gravity4.facebook.dpareports.api.Country;
...
import com.gravity4.facebook.dpareports.CSVFileWriter.Country.CountryCampaignCSVWriter;
import com.gravity4.facebook.dpareports.dao.Country.CountryCampaignLevelStatsDAO;
import com.gravity4.facebook.dpareports.model.CSVModels.Country.CSVCountryCampaignStats;
import com.gravity4.facebook.dpareports.model.APImodels.CountryandRegion.CountryandRegionCampaignStatsLoader;
import com.gravity4.facebook.dpareports.responseparser.responsedata.Country.CountryCampaignStatsJSONResponse;
import com.gravity4.facebook.dpareports.responseparser.resultdata.Country.CountryCampaignResultData;
...
public class CountryCampaignStats {

    Logger logger = LoggerFactory.getLogger(CountryCampaignStats.class);


    @Autowired
    CountryCampaignLevelStatsDAO countryCampaignLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getCountryCampaignstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_group_name','spend','country','total_actions'," +
         ...
         */
            if (status) {
                ...
                CountryCampaignStatsJSONResponse response = gson.fromJson(jsonfeed, CountryCampaignStatsJSONResponse.class);

                List<CountryCampaignResultData> results = response.data;

                CountryandRegionCampaignStatsLoader campaignStatsLoader;
                List<CountryandRegionCampaignStatsLoader> campaignStatsLoaderList = new ArrayList<CountryandRegionCampaignStatsLoader>();

                for (CountryCampaignResultData resultData : results) {

                    campaignStatsLoader = new CountryandRegionCampaignStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    campaignStatsLoader.setRegion("");

                    campaignStatsLoaderList.add(campaignStatsLoader);


                }
                countryCampaignLevelStatsDAO.storecamapignlevelstats(campaignStatsLoaderList);

                //for storing into file for email
                List<CSVCountryCampaignStats> csvCountryCampaignStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvCountryCampaignStatsList=countryCampaignLevelStatsDAO.fileadsetlevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= CountryCampaignCSVWriter.writecsvfile(csvCountryCampaignStatsList, Client_ID, emailstatsdate,csv_address);
