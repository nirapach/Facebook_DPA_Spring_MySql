...
import com.gravity4.facebook.dpareports.CSVFileWriter.Aggregated.AggregatedAdSetCSVWriter;
import com.gravity4.facebook.dpareports.dao.AggregatedandAgeandGender.OverAllAdSetLevelStatsDAO;
import com.gravity4.facebook.dpareports.model.APImodels.AggregatedandProductLevel.AdSetStatsLoader;
import com.gravity4.facebook.dpareports.model.CSVModels.Aggregated.CSVAggregatedAdSetStats;
import com.gravity4.facebook.dpareports.responseparser.responsedata.Aggregated.AggregatedAdSetStatsJSONResponse;
...
public class AggregatedAdSetStats {

    Logger logger = LoggerFactory.getLogger(AggregatedAdSetStats.class);
    ...
    /*
        ...
        */
    public String getOverAllAdSetstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['campaign_name','spend','total_actions'," +
         ...
         */
            if (status) {
                ...
                AggregatedAdSetStatsJSONResponse response = gson.fromJson(jsonfeed, AggregatedAdSetStatsJSONResponse.class);

                List<AggregatedAdSetResultData> results = response.data;

                AdSetStatsLoader adSetStatsLoader;
                List<AdSetStatsLoader> adSetStatsLoaderList = new ArrayList<AdSetStatsLoader>();

                for (AggregatedAdSetResultData resultData : results) {
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    adSetStatsLoader.setGender("");

                    adSetStatsLoaderList.add(adSetStatsLoader);


                }
                overAllAdSetLevelStatsDAO.storeaggregatedadsetlevelstats(adSetStatsLoaderList);


                //for storing into file for email
                List<CSVAggregatedAdSetStats> csvAggregatedAdSetStatsList;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvAggregatedAdSetStatsList=overAllAdSetLevelStatsDAO.fileaggregatedadsetlevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= AggregatedAdSetCSVWriter.writecsvfile(csvAggregatedAdSetStatsList, Client_ID, emailstatsdate,csv_address);
