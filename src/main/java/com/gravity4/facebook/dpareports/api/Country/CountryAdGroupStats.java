...
import com.gravity4.facebook.dpareports.CSVFileWriter.Country.CountryAdGroupCSVWriter;
import com.gravity4.facebook.dpareports.dao.Country.CountryAdGroupLevelStatsDAO;
import com.gravity4.facebook.dpareports.model.CSVModels.Country.CSVCountryAdGroupStats;
import com.gravity4.facebook.dpareports.model.APImodels.CountryandRegion.CountryAndRegionAdGroupStatsLoader;
import com.gravity4.facebook.dpareports.responseparser.responsedata.Country.CountryAdGroupStatsJSONResponse;
...
public class CountryAdGroupStats {

    Logger logger = LoggerFactory.getLogger(CountryAdGroupStats.class);

    @Autowired
    CountryAdGroupLevelStatsDAO countryAdGroupLevelStatsDAO;
    ...
    /*
        ...
        */
    public String getCountryAdGroupstats(long Account_ID_Integer, long Client_ID_Integer, String Access_Token,String csv_address) throws URISyntaxException, IOException, PropertyVetoException, SQLException, HTTPException {
        ...
        String data_columns = "['adgroup_name','spend','country','total_actions'," +
         ...
         */
            if (status) {
                ...
                CountryAdGroupStatsJSONResponse response = gson.fromJson(jsonfeed, CountryAdGroupStatsJSONResponse.class);

                List<CountryAdGroupResultData> results = response.data;

                CountryAndRegionAdGroupStatsLoader adGroupStatsLoader;
                List<CountryAndRegionAdGroupStatsLoader> adGroupStatsLoaderList = new ArrayList<CountryAndRegionAdGroupStatsLoader>();

                for (CountryAdGroupResultData resultData : results) {

                    adGroupStatsLoader = new CountryAndRegionAdGroupStatsLoader();
            ...
            are getting yesterday's datein date_preset field of the curl request*/
                    ...
                    adGroupStatsLoader.setRegion("");

                    adGroupStatsLoaderList.add(adGroupStatsLoader);


                }
                countryAdGroupLevelStatsDAO.storeadgrouplevelstats(adGroupStatsLoaderList);

                //for storing into file for email
                List<CSVCountryAdGroupStats> csvCountryAdGroupStatslist;
                java.sql.Date emailstatsdate = new java.sql.Date(StatisticsDate.getYesterday().getTime());
                csvCountryAdGroupStatslist=countryAdGroupLevelStatsDAO.fileadgrouplevelstats(Client_ID, emailstatsdate);
                //call the CSV file writer
                String stored= CountryAdGroupCSVWriter.writecsvfile(csvCountryAdGroupStatslist, Client_ID, emailstatsdate,csv_address);
