...
import com.gravity4.facebook.dpareports.mapper.Aggregated.AggregatedAdSetCSVMapper;
import com.gravity4.facebook.dpareports.mapper.AgeandGender.OverAllAdSetCSVMapper;
...
public class OverAllAdSetLevelStatsDAO extends BaseDAO {
    ...
    public List<CSVAggregatedAdSetStats> fileaggregatedadsetlevelstats(final long page_id,Date Stats_date) {

        List<CSVAggregatedAdSetStats> CSVOverAllAdSetStats;
        String SELECT = " SELECT * "
                + " FROM Aggregated_AdSet_Statistics_Results"
                + " WHERE Application_Client_ID="+page_id+" and Stats_Date="+"'"+Stats_date+"'";

        CSVOverAllAdSetStats=getJdbcTemplate().query(SELECT,new AggregatedAdSetCSVMapper());
    ...
    public void storeaggregatedadsetlevelstats(final List<AdSetStatsLoader> adSetStatsLoaderList){

        String query = "INSERT INTO Aggregated_AdSet_Statistics_Results" +
                "(Application_Client_ID," +
                "Application_Ad_AdSet_ID," +
                "Application_Ad_AdSet_Name," +
                "Client_Reports_Reach," +
        ...
        getJdbcTemplate().batchUpdate(query, new BatchPreparedStatementSetter() {
            ...
            public void setValues(PreparedStatement statement, int i) throws SQLException {
                ...
                statement.setDouble(21, adSetStatsLoader.getCost_Per_Unique_Click());
